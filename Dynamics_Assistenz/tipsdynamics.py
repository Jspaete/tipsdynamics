...
class Tip():
    ...
    def define(self):
        ...
        while definition_stop != 'yes':
            self.tip += [input('give now '+ str(tip_nmbr)+'. tip: ')]
            self.picture += [input('picture? give file name: ')]
            tip_nmbr+=1
    ...
    def get_tip(self,tip_nmbr):
        if self.tip[tip_nmbr] != '0':
            print(self.tip[tip_nmbr])
        if self.picture[tip_nmbr] != '0':
            img = Image.open(self.picture[tip_nmbr])
            img.show()

    def check_tip(self,tip_nmbr):
        if len(self.tip) > tip_nmbr or len(self.picture) > tip_nmbr:
            return 1
        return 0
...
def load_w(week):
    tips_container = {}
    #am ende dieser definition wird filenames wieder gelöscht
    filenames = glob.glob(f"w{week}*.pickle") 
